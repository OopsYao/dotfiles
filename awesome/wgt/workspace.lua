...
local icons = gears.table.map(
    function(icon)
        return os.getenv('HOME') .. '/.config/awesome/icons/' .. icon
    end, {
        'archlinux.svg', 'firefox.svg', 'code.svg', 'quote.svg', 'spotify.svg',
        'docker.svg', 'paragraph.svg', 'github.svg', 'chrome.svg', 'neovim.svg'
    }
)
local update_tags = function(self, tag, init)
    ...
    if indicator == 10 then indicator = 0 end
    text_role.markup = '<span color="' .. color .. '">' .. indicator ..
                           '</span>'
end
