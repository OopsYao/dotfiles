...
return function()
  ...
  awful.spawn.with_line_callback('playerctl metadata -F --format "' .. playerctl_format .. '"', {
    stdout = function(line)
      if line ~= "" then
        local info = parse_playerctl_output(line)
        player:get_children_by_id("info")[1].text = info.title .. " - " .. info.artist
        url2local(info.cover, function(local_path)
  ...
  awful.spawn.with_line_callback("playerctl status -F", {
    stdout = function(line)
      if line == "" then
        -- Player exits
